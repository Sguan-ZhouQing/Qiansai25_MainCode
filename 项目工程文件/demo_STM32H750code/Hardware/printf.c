 ...
 * @LastEditTime: 2025-07-17 22:12:58
...
extern uint8_t F407_readBuffer[10];
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    if (huart->Instance == USART1) {
        for(uint16_t i = 0; i < sizeof(F407_readBuffer); i++) {
            // 遇到结束符停止检查
            if(F407_readBuffer[i] == '\0' || F407_readBuffer[i] == '\r' || F407_readBuffer[i] == '\n') {
                break;
            }
            // 检查是否是0xB0命令
            if(F407_readBuffer[i] == 0xC1) {
                uint8_t own_data = 0xC2;
                HAL_UART_Transmit(&huart1,&own_data,sizeof(own_data),HAL_MAX_DELAY);
            }
        }
        // 重新启动接收，使用Ex函数，接收不定长数据
        HAL_UARTEx_ReceiveToIdle_DMA(&huart1, F407_readBuffer, sizeof(F407_readBuffer));
