 ...
 * @LastEditTime: 2025-07-10 03:54:51
...
// 获取人体距离（返回厘米数，如果没有检测到目标返回0）
void GetHumanDistance_Tick(void) {
    HLK_Data_t data;
    // 接收数据
    if (HLK24G_ReceiveData(&data)) {
        // 根据目标状态返回适当的距离值
        switch (data.status) {
            case HLK_TARGET_MOVING:
                my_distance = data.moving_distance;
            case HLK_TARGET_STATIC:
                my_distance = data.static_distance;
            case HLK_TARGET_BOTH:
                // 如果有两种目标，返回较近的一个
                my_distance = (data.moving_distance < data.static_distance) ? 
                       data.moving_distance : data.static_distance;
            case HLK_TARGET_NONE:
            default:
                my_distance = 0;
        }
    }
    ESP_SendMQTTVar("distance",my_distance);
    osDelay(1000);
}
